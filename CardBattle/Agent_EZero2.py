...
for episode in range(episodes+1):
    ...
    for i in range(MaxSteps):
        ...
        if Memory_1P.length() > batch_size and Memory_2P.length() > batch_size:
            ...
            ret_action2P = Model2P(torch.from_numpy(Load_Inputs2P).to(Devise).float())
        ...
        if CurrentStep > JustLooking:
            ...
            Experience1P.extend(State1P[0].tolist())
            Experience1P.extend(action1P.tolist())
            Experience1P.extend(Reward1P)
            Experience1P.extend(NextState1P[0].tolist())
            #2Pも同様に格納
            Reward2P = [float(Reward2P)]
            Experience2P.extend(State2P[0].tolist())
            Experience2P.extend(action2P.tolist())
            Experience2P.extend(Reward2P)
            Experience2P.extend(NextState2P[0].tolist())
        ...
        if Memory_1P.length() > batch_size:
            ...
            for Input_itr in Inputs1P:
                if hasattr(Input_itr, "__iter__"):
                    ...
                    Load_Inputs1P.append(Input_itr)
            Load_Inputs1P = np.array(Load_Inputs1P,dtype=np.float32)
            Output_Train = Model1P(torch.from_numpy(Load_Inputs1P).to(Devise).float())
            Output_Target = Model1P_Target(torch.from_numpy(Load_Inputs1P).to(Devise).float())
            loss1P = criterion(Output_Train,Output_Target)
            optimizer1P.zero_grad()
            loss1P.backward(retain_graph=True)
            optimizer1P.step()
        if Memory_2P.length() > batch_size:
            ...
            for Input_itr in Inputs2P:
                if hasattr(Input_itr, "__iter__"):
                    ...
                    Load_Inputs2P.append(Input_itr)
            Load_Inputs2P = np.array(Load_Inputs2P,dtype=np.float32)
            Output_Train = Model2P(torch.from_numpy(Load_Inputs2P).to(Devise).float())
            Output_Target = Model2P_Target(torch.from_numpy(Load_Inputs2P).to(Devise).float())
            loss2P = criterion(Output_Train,Output_Target)
            optimizer2P.zero_grad()
            loss2P.backward(retain_graph=True)
            optimizer2P.step()
        #エピソード完了時
